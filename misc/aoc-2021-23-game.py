...
class Burrow:
    pods = {
        (2, 1): Amphipod(data[31]),
        (4, 1): Amphipod(data[33]),
        (6, 1): Amphipod(data[35]),
        (8, 1): Amphipod(data[37]),
        (2, 2): Amphipod(data[45]),
        (4, 2): Amphipod(data[47]),
        (6, 2): Amphipod(data[49]),
        (8, 2): Amphipod(data[51]),
    }
    pods2 = {
        (2, 2): Amphipod("D"),
        (4, 2): Amphipod("C"),
    ...
    def __init__(self, part):
        ...
        for pos in self.pods:
            self.__spaces[
                (pos[0], 4) if part == 2 and pos[1] == 2 else pos
            ].setOccupant(self.pods[pos])
        if part == 2:
    ...
    def get_size(self):
        return (
            max(self.__spaces.keys(), key=lambda x: x[0])[0] + 1,
            max(self.__spaces.keys(), key=lambda x: x[1])[1] + 1,
        )
    ...
    def move(self, direction):
        new_position = (
            self.__selected[0] + direction[0],
            self.__selected[1] + direction[1],
        )
...
while running:
    ...
    for space in burrow.get_spaces():
        pygame.draw.rect(
            ...
            Color.SPACE_OUTLINE,
            (
                space.getPosition()[0] * TILE_SIZE + 9,
                space.getPosition()[1] * TILE_SIZE + 59,
                TILE_SIZE - 8,
                TILE_SIZE - 8,
            ),
        )
        pygame.draw.rect(
            ...
            Color.SPACE_FILL,
            (
                space.getPosition()[0] * TILE_SIZE + 10,
                space.getPosition()[1] * TILE_SIZE + 60,
                TILE_SIZE - 10,
                TILE_SIZE - 10,
            ),
        )
        ...
        if not space.isOccupied and len(space.type) == 1:
            ...
            screen.blit(
                text,
                (
                    space.getPosition()[0] * TILE_SIZE + 20,
                    space.getPosition()[1] * TILE_SIZE + 52,
                ),
            )
    ...
    screen.blit(
        help_text,
        help_text.get_rect(center=(SCREEN_WIDTH / 2, 0)).move(0, TILE_SIZE * 7 - 20),
    )
    help_text_2 = help_font.render(HELP_TEXT_2, True, Color.TEXT_WHITE)
    screen.blit(
        help_text_2,
        help_text_2.get_rect(center=(SCREEN_WIDTH / 2, 0)).move(0, TILE_SIZE * 7 + 10),
    )
